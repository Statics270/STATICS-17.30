...
namespace FortGameModeAthena {
    ...
    APawn* SpawnDefaultPawnFor(AFortGameModeAthena* GameMode, AFortPlayerControllerAthena* NewPlayer, AActor* StartSpot) {
        ...
        if (NewPlayer->WorldInventory) {
            NewPlayer->WorldInventory->Inventory.ReplicatedEntries.Clear();
            NewPlayer->WorldInventory->Inventory.ItemInstances.Clear();
            NewPlayer->WorldInventory->HandleInventoryLocalUpdate();
            Log("Cleared previous match inventory!");
        }
